...
impl Field for CST_570004_WW_Fields {
    fn id(&self) -> u16 {
        match self {
    ...
    fn read_callback(&self, _v: String) -> Option<u16> {
        // eprintln!("{} read_callback v: {}", self.name(), v);
        match self {
    ...
    fn pre_write_xform_set_property(&self, v: String) -> Option<(String, String)> {
        // eprintln!("{} pre_write_xform_set_property v: {}", self.name(), v);
        match self {
            Self::Mode => {
                if v == "off" {
                    Some((String::from("power"), String::from("OFF")))
                } else {
                    Some((String::from("power"), String::from("ON")))
                }
            }
            _ => None,
        }
    ...
    fn write_attach(&self, raw: u32) -> Option<Vec<u16>> {
        // eprintln!("{} write_attach raw: {}", self.name(), raw);
        match self {
...
#[allow(non_camel_case_types)]
#[derive(Clone)]
pub struct CST_570004_WW;

impl HADevice for CST_570004_WW {
    fn get_ha_class(&self) -> String {
        String::from("climate")
    }

    fn get_model(&self) -> String {
        String::from("CST_570004_WW")
    }
